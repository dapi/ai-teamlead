...
агентом поверх реального LLM в полностью изолированном sandbox.
...
- умеет запускать настоящего агента (`claude` по умолчанию, `codex` как
...
- `claude` с моделью класса Sonnet как default live path для локального
  тестирования
...
- два режима агента:
  ...
  - `live` для запуска реального `claude` с Sonnet по умолчанию и `codex` как
...
- явный bridge для host env vars и host config files, нужных агенту для доступа
  к LLM API; bridge использует те же host-level значения, с которыми запущен
  test suite
...
- под "локальными настройками" в этой feature имеются в виду именно host-level
  настройки, credentials и параметры подключения, с которыми запущен сам test
  suite
