...
План не должен выходить на ревью человеком, пока не завершены обязательные
исследования, не закрыты вопросы, влияющие на реализацию, и пока верхнеуровневые
шаги не привязаны к текущему коду, runtime-контрактам и используемым
зависимостям целевой системы.
...
- `plan-ready`:
  - допустим только если обязательные исследования завершены, все вопросы,
    влияющие на реализацию, закрыты, а план привязан к текущей системе
    достаточно глубоко для надежного порядка реализации
...
- содержать явный план изменений документации: какие канонические документы,
  ...
  изменений
- явно фиксировать, что все исследования завершены, все вопросы, влияющие на
  реализацию, закрыты на стадии анализа, а ключевые шаги привязаны к текущей
  системе
...
- быть достаточно однозначным, чтобы стадия реализации не требовала нового
  исследовательского цикла и дополнительных уточнений у пользователя
