...
pub fn plan_agent_flow_test(
    ...
    let artifacts_dir = request
        ...
        .unwrap_or_else(|| default_artifacts_dir(git_dir));
...
fn map_container_mount_path(agent: AgentFlowAgent, host_path: &Path) -> Option<String> {
    ...
    match agent {
        ...
        AgentFlowAgent::Claude => {
            ...
            } else if value.ends_with("/.claude.json")
                || host_path.file_name().and_then(|v| v.to_str()) == Some(".claude.json")
            {
...
"#,
    ...
    match plan.agent {
        ...
        AgentFlowAgent::Claude => {
            ...
            script.push_str(
                "for file in .claude.json .claude.mcp-disabled.json; do if [[ -f \"/input/live-home/$file\" ]]; then cp -a \"/input/live-home/$file\" \"$HOME/$file\"; fi; done\n",
            );
            script.push_str(
            ...
            script.push_str("mv \"$tmp_claude_state\" \"$HOME/.claude.json\"\n");
            script.push_str(
...
capture_session_progress() {
  ...
  local session_uuid
  session_uuid="$(jq -r '.session_uuid // .bindings.analysis // .bindings.implementation // empty' "$issue_index")"
...
"#,
    ...
    fn sandbox_run_for_live_claude_uses_agent_profile_and_mounts() {
        ...
        let claude_state = temp.path().join(".claude.json");
        let claude_mcp_state = temp.path().join(".claude.mcp-disabled.json");
        ...
        fs::write(&claude_state, "{\"hasCompletedOnboarding\":true}\n").expect("claude state");
        fs::write(&claude_mcp_state, "{}\n").expect("claude mcp state");
        fs::write(&fake_binary, "#!/usr/bin/env bash\n").expect("binary");
        fs::write(
        ...
        assert!(command.contains(&format!(
            "-v {}:/input/live-home/.claude.json:ro",
            claude_state.display()
        )));
        assert!(command.contains(&format!(
            "-v {}:/input/live-home/.claude.mcp-disabled.json:ro",
            claude_mcp_state.display()
        )));
        assert!(command.contains(&format!(
