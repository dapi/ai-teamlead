...
if grep -Fq '#   launch_target: "pane"' "$SETTINGS_FILE"; then
    echo "  PASS: init documents launch_target runtime default as pane"
    ((PASS++)) || true
else
    echo "  FAIL: init documents launch_target runtime default as pane"
    ((FAIL++)) || true
fi
