...
ошибочных ситуаций и никак не пытается их автоматически исправить:
...
- `run <issue>` различает минимум четыре operator-facing ветки:
  `issue not found`, `issue closed`, `issue not in target project`,
  `issue status is not a valid run entry point`;
- для `issue not in target project` система сначала пытается сама добавить
  issue в configured GitHub Project и выставить стартовый status, а пользователю
  сообщает уже результат этого исправления;
- для недопустимого статуса система сначала пытается сама перевести issue в
  корректный entry status для stage-aware `run`, а при невозможности объясняет,
  почему автоисправление не выполнено и какой статус нужно выставить вручную;
- сообщение для отказных веток, которые нельзя исправить автоматически,
  содержит причину, текущий факт состояния и явный ручной fallback;
- доменный слой формирует не только user-facing сообщение, но и
  структурированное remediation-решение отдельно от orchestration-кода, чтобы
  его можно было покрыть unit-тестами;
